...
class Generate_pod_model extends CI_Model {
	...
	{
		$this->db->select('tbl_domestic_booking.reciever_pincode,tbl_domestic_booking.reciever_address,tbl_domestic_booking.booking_id,tbl_domestic_booking.company_type,city.city as reciever_city,mode_dispatch,transfer_mode.mode_name,max(tbl_domestic_tracking.tracking_date) as tracking_date,comment,remarks,booking_date,tbl_domestic_booking.pod_no,sender_name,doc_nondoc,city.city,tbl_domestic_booking.forwording_no,tbl_domestic_booking.forworder_name,reciever_name,dispatch_details,chargable_weight,max(tbl_domestic_tracking.status) as status,tbl_domestic_booking.is_delhivery_complete,delivery_date,tbl_domestic_booking.customer_id,no_of_pack,tbl_branch.branch_name,invoice_no,invoice_value');
