...
class Vendor_controller extends CI_Controller
{
  ...
  { 
   ...
   $id = $this->session->userdata('customer_id');
    $this->load->view('vendor/include/header');
    $data['total_order'] = $this->db->query(" SELECT count(ftl_request_id) as total  FROM `ftl_request_tbl` WHERE ftl_booking_status = 0")->result_array();
  ...
  public function ftl_request_for_vendor($id)
  {
    ...
    $this->load->view('vendor/include/header');
    // $id = $this->session->userdata('customer_id');
    $data['ftl_request_data'] = $this->db->query("select * from ftl_request_tbl where  id = '$id' AND ftl_booking_status ='0'")->result_array();
  ...
  ///print_r($data);exit;2023-02-20 10:59:57
    ...
    if(!empty($origin_pincode)){
     // print_r($origin_pincode);exit;
      $get_service_pincode = $this->db->query("select branch_id from tbl_branch_service  where pincode = '$origin_pincode'")->row_array();
     // echo $this->db->last_query();exit;
    $branch =  $get_service_pincode['branch_id'];
    //print_r($branch);exit;

      $get_trafic_user = $this->db->query("select * from tbl_users  where branch_id ='$branch' AND user_type ='28'")->row_array();
      $user_id = $get_trafic_user['user_id'];
      $branch_id = $get_trafic_user['branch_id'];

       $this->db->query("update order_request_tabel  set trafic_branch_id = '$branch_id' where id ='$last_id'");
     //  echo $this->db->last_query();exit;
    }
