...
*/
	...
	public function check_uploaded_pod(){
		$all_data 		= $this->input->post();

		$pod_no = @$all_data['pod_no'];
		if (!empty($all_data) && !empty($pod_no)) 
		{
			$user_id=@$all_data['user_id'];
			$pod_no=$all_data['pod_no'];
			$whr = array('user_id'=>$user_id);
			...
			$whr = array('pod_no'=>$pod_no);
			$res5=$this->basic_operation_m->getAll('tbl_domestic_booking',$whr);

			// echo $this->db->last_query();
			$res=$this->basic_operation_m->getAll('tbl_upload_pod',$whr);
			$image= @$res->row()->image;
			$pod_no_check= @$res5->row()->pod_no;

			if (!empty($image)) {
				$data['status']="already";
				$data['message']="Already Exist";
			}else if(empty($pod_no_check)){
				$data['status']="faild";
				$data['message']="Not A Valid POD No";
			}else{
				$data['status']="success";
