...
class Admin_incoming extends CI_Controller{
	...
	{  
		...
		$resAct=$this->db->query("select  *,SUM(CASE WHEN reciving_status=1 THEN 1 ELSE 0 END)  AS total_coming, COUNT(bag_no) AS total, COUNT(bag_no) AS total_pcs, SUM(total_weight) AS total_weight from tbl_domestic_menifiest where tbl_domestic_menifiest.destination_branch='$branch_name' group by manifiest_id order by date_added DESC");
	...
	public function addincoming($id='')
	{
		...
		//  $resAct=$this->db->query("select distinct manifiest_id,date_added from tbl_domestic_menifiest where destination_branch='$branch_name'");
		 $resAct=$this->db->query("select * from tbl_domestic_menifiest where destination_branch='$branch_name' group by manifiest_id");
        if($resAct->num_rows()>0)
        {
				...
				foreach($all_data as $key => $values)
				{
					if($key !== 'manifiest_id' && $key !== 'receving'	 && $key !== 'datetime'	)
					{
						...
						if(!empty($values))
						{
							...
							//$resAct		=	$this->db->query("update tbl_domestic_bag set menifiest_recived = '0' where pod_no='$pod_no'");
