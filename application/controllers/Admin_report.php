...
class Admin_report extends CI_Controller {
	...
	{    
		...
		$header =array("SrNo","Date","Booking Branch","AWB","Network","Type","ForwordingNo","Destination","Customer","Receiver","Receiver Addr","Receiver Pincode","Doc/Non-doc","Weight","Bill Type","Status","Branch","Delivery Date","EDD","TAT","Deliverd TO","RTO Date","RTO Reason");
		...
		foreach($domestic_allpoddata as $value_d) 
		{
			...
			if($value_d['status'] == 'shifted'){
               ...
               $value_d['status'] = 'In-transit';
			}

			//print_r($value_d['status']);

			$row=array($i,date('d-m-Y', strtotime($value_d['booking_date'])),$value_d['branch_name'],$value_d['pod_no'],$value_d['forworder_name'],$value_d['company_type'],$value_d['forwording_no'],$value_d['city'],$value_d['sender_name'],$value_d['reciever_name'],$value_d['reciever_address'],$value_d['reciever_pincode'],$value_d['doc_nondoc'],($value_d['chargable_weight']),$value_d['dispatch_details'],$value_d['status'],$value_d['branch_name'],$delivery_date,$value_d['delivery_date'],$tat,$value_d['comment'],$rto_date,$rto_reason);
