...
use Dompdf\Dompdf;
class Admin_international_import_booking extends CI_Controller {
	...
	public function index() {
			...
			if (isset($_POST['submit'])) 
			{
				...
				$where = array('customer_id'=>$this->input->post('customer_account_id'),
					'import_invoice_generated_status'=>0,
			...
			if (isset($_POST['submit_print'])) 
			{
					...
					$toDate = $this->input->post('to');
		             ...
		             $max_number= $this->basic_operation_m->get_max_number('tbl_international_invoice_import','MAX(inc_num) AS id');
		             ...
		             $invoice_series = $data['company_details']->import_international_invoice_series ;
					...
					$invoice['export_import_type']='Import';
					...
					$invoice['booking_ids'] = json_encode($this->input->post('selected_dockets') );
					 $this->db->insert('tbl_international_invoice_import', $invoice);
	                ...
	                $grand_total=0;
					 ...
					 foreach ($data['allpoddata'] as $pod)
					 {
		                   ...
		                   $d_update =array('import_invoice_generated_status'=>'1');
		                   ...
		                   // echo "<pre>";print_r($invoice_detail);
		                    $this->db->insert('tbl_international_invoice_detail_import', $invoice_detail);
		               ...
		               $this->db->update('tbl_international_invoice_import', $invoiceData);
		               unset($invoiceData);

					redirect("admin/list-invoice-import");
		 ...
		 $this->load->view('admin/booking_master/Import/booking_invoice', $data);
	}
	...
	{
    	$data['id'] 		= $id;	
		$data['customer'] 	= $this->basic_operation_m->get_table_row('tbl_international_invoice_import',array('id'=>$id));
		$data['allpoddata'] = $this->basic_operation_m->get_all_result('tbl_international_invoice_detail_import',array('invoice_id'=>$id));
		$data['gstCharges'] = $this->basic_operation_m->get_query_row('select * from tbl_gst_setting order by id desc limit 1');
		$this->load->view('admin/booking_master/Import/edit_invoice', $data);
    }
    public function final_invoice_edit($id,$customer_id) {
    	...
    	$this->load->view('admin/booking_master/Import/edit_final_invoice', $data);
    }
   public function invoice_view($id,$company_id) {
    	$data['id']	 = $id;
    	$in 				= $this->db->query("select * from tbl_international_invoice_import where id = ". $id." ORDER BY inc_num DESC  ");
    	$data['customer'] 	= $in->row();
    	$in_de 				= $this->db->query("select * from tbl_international_invoice_detail_import where invoice_id = ". $id." Order by booking_date ASC" );
    	...
    	$this->load->view('admin/booking_master/Import/billing_invoice_view', $data);
    }

	public function pdf($id,$company_id) {
    	$data['id'] = $id;
    	$in = $this->db->query("select * from tbl_international_invoice_import where id = ". $id."  ");
    	$data['customer'] = $in->row();
    	$inc_num =$data['customer']->inc_num;
    	
    	$in_de = $this->db->query("select * from tbl_international_invoice_detail_import where invoice_id = ". $id." Order by booking_date ASC" );
    	$data['allpoddata'] = $in_de->result_array();
    	$gstCharges = $this->db->query("select * from setting");
    	$data['gstCharges'] = $gstCharges->result_array();
    	...
    	// $html = $this->load->view('admin/booking_master/Import/booking_print', $data, true);
    	$html = $this->load->view('admin/booking_master/Import/booking_pdf', $data, true);
		// echo $html; die;
	    ...
	    $this->m_pdf->pdf->defaultheaderfontsize=12;
		...
		$savefolderpath = 'assets/invoice/international_import/';
	...
	public function get_invoiceNumber($invoicePrefix){
		...
		$in = $this->db->query("select invoice_prefix from tbl_international_invoice_import where invoice_prefix = '$invoicePrefix'");
	...
	{		
		...
		$in = $this->db->query("select invoice_number,company_id from tbl_international_invoice_import where id = '$id'");
    	...
    	$this->db->where('id', $id);
    	$this->db->update('tbl_international_invoice_import', $invoice);
	
		$list = $this->input->post('list');
		
      
        $query = "select tbl_international_invoice_import.*, tbl_customers.pincode, tbl_customers.phone, tbl_customers.sac_code from tbl_international_invoice_import LEFT JOIN tbl_customers ON tbl_international_invoice_import .cid = tbl_customers.cid where tbl_international_invoice_import.id =".$id;
        
        $data['customer'] = $this->basic_operation_m->get_query_row($query);
        $inc_num =$data['customer']->inc_num;
        $company_id =$data['customer']->company_id;
       
        $in_de = "select tbl_international_invoice_detail_import.*, tbl_international_booking.invoice_no as booking_invoice_num from tbl_international_invoice_detail_import INNER JOIN tbl_international_booking ON tbl_international_invoice_detail_import.booking_id = tbl_international_booking.booking_id where tbl_international_invoice_detail_import.invoice_id =".$id;
		...
		{
			redirect('admin/list-finalInvoice-import'); 
		}
		else
		{
			redirect('admin/list-invoice-import'); 
...
// 	public function save_inv_deatails_ajax(){
// 		//print_r($_POST); 
...
// 		$dod_doac_charges = $this->input->post('dod_doac');
...
// 		if($dod_doac_charges == '' || $dod_doac_charges == '-'){
// 			$dod_doac_charges = 0;
// 		}

// 		//$save['invoice_id'] = 
// 		$save['pod_no'] = $pod;
// 		$save['reciever_name'] = $reciever_name;
// 		$save['reciever_city'] = $reciever_city;
// 		$save['mode_dispatch'] = $mode_dispatch;
// 		$save['no_of_pack'] = $no_of_pack;
// 		$save['rate'] = $rate;
// 		$save['chargable_weight'] = $chargable_weight;
		
// 		$save['to_pay'] = $to_pay;
// 		$save['fov'] = $fov;
// 		$save['oda_charges'] = $oda_charges;
// 		$save['dod_doac'] = $dod_doac_charges;
// 		$save['frieht'] = $frieht;
// 		$save['fuel'] = $fuel;
// 		$save['other_charges'] = $other_charges;
// 		$save['sub_total'] = $total_amount;
// 		$save['amount'] = $total_amount;

// 		$total_charge_amount = $to_pay  + $oda_charges + $dod_doac_charges + $frieht + $fov;
		
// 		$this->db->where('id', $id);
// 		$this->db->update('tbl_invoice_detail', $save);
// 		return true;
// 	}
    ...
    public function invoice() {    	
    		...
    		$this->load->helper('form_helper');
    		if ($this->session->userdata("userType") == '1') {
    			$resAct = $this->db->query("select * from tbl_international_invoice_import where final_invoice = 0");
    			...
    			$resAct = $this->db->query("select * from tbl_international_invoice_detail_import where branch_id='$branch_id' and final_invoice = 0 ORDER BY tbl_international_invoice_import.id desc");
    			if ($resAct->num_rows() > 0) {
    				$data['allpoddata'] = $resAct->result_array();
    			}
    		}
    		$this->load->view('admin/booking_master/Import/view_invoice', $data);
    ...
    public function finalInvoice() {
    		...
    		if ($this->session->userdata("userType") == '1') {
    			...
    			$data['allpoddata'] = $this->booking_model->select_import_invoice_details($whrCon);
    		} else 
    		{
	        ...
	        $data['prefix'] = $data['company_details']->import_international_invoice_series;
	        
    		$this->load->view('admin/booking_master/Import/view_invoice_final_list', $data);
	 ...
	 public function send_mail($invoice_id, $customer_id) 
	{
		...
		$inv_details = $this->db->query("select invoice_number,inc_num,company_id from tbl_international_invoice_import WHERE id='$invoice_id'");
		...
		$where =array('id'=>$company_id);
		$data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);
	    $prefix = $data['company_details']->import_international_invoice_series;
...
//echo $message;
          ...
          $this->email->attach(base_url('assets/invoice/international_import/' . $name));
          if($this->email->send())
         {
             $data=array('send_mail_status'=>1);
             $whr =array('id'=>$invoice_id,'customer_id'=>$customer_id);
             $this->basic_operation_m->update('tbl_international_invoice_import',$data,$whr);
         }
          redirect('admin/list-finalInvoice-import');
    } 
      ...
      public function invoice_delete($id) {
      	$this->db->where('id', $id);
      	$this->db->delete('tbl_international_invoice_import');
      	
      	$whr =array('invoice_id'=> $id);
      	$booking_id_details = $this->basic_operation_m->get_all_result("tbl_international_invoice_detail_import",$whr);
      
		foreach ($booking_id_details as $value) {
			$d_update =array('import_invoice_generated_status'=>'0');
 			$d_whr=array('booking_id'=>$value['booking_id']);
			  	$this->basic_operation_m->update('tbl_international_booking',$d_update,$d_whr);
		}
   
        $this->db->where('id', $id);
      	$this->db->delete('tbl_international_invoice_detail_import');
      	redirect('admin/list-invoice-import');
      }

      public function international_excel($invoiceId)
      {
           $where =array('id'=>1);
            $data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);
            $invoice_series = $data['company_details']->import_international_invoice_series;
			$filename = $invoice_series."_".$invoiceId.".csv";
			$fp = fopen('php://output', 'w');
			$header =array(
				"NO.","Date","DOC NO.","CONSIGNEE NAME","INVOICE NO","INVOICE VALUE","DESTINATION","MODE","NO PCS",
				"WEIGHT","TRANSPORT CH","DESTINATION CH","CLEARANCE CH","ECS","OTHER CH","TOTAL AMOUNT");

			$invoice_data = $this->Invoice_model->getImportInvoice($invoiceId);
			$invoiceDeatil_data = $this->Invoice_model->getImportInvoiceDetail($invoiceId);
				
			header('Content-type: application/csv');
			header('Content-Disposition: attachment; filename='.$filename);

			$row =array($invoice_data->customer_name);
			fputcsv($fp, $row);
// 			$row =array($invoice_data->address);
// 			fputcsv($fp, $row);
			$row =array('Invoice Number',$invoice_data->invoice_number);
			fputcsv($fp, $row);
			$row =array('Gst Number',$invoice_data->gstno);
			fputcsv($fp, $row);
			$row =array('Invoice Date',date('d-m-Y',strtotime($invoice_data->created_on)));
			fputcsv($fp, $row);
			$row =array('Performa Number',$invoice_data->performa_invoice_number);
			fputcsv($fp, $row);
			...
			foreach($invoiceDeatil_data as $row) {
				$i++;
				$row=array($i,$row->booking_date,$row->pod_no,$row->reciever_name,$row->booking_invoice_num,$row->invoice_value,$row->reciever_country,$row->mode_dispatch,$row->no_of_pack,$row->chargable_weight,$row->transportation_charges,$row->destination_charges,$row->clearance_charges,$row->ecs,$row->other_charges,$row->sub_total);
				fputcsv($fp, $row);
			}
			$row=array('','','','','','','','','','','','','','','Sub Total',$invoice_data->sub_total);
			fputcsv($fp, $row);
			$row=array('','','','','','','','','','','','','','','CGST',$invoice_data->cgst_amount);
			fputcsv($fp, $row);
			$row=array('','','','','','','','','','','','','','','SGST',$invoice_data->sgst_amount);
			fputcsv($fp, $row);
			$row=array('','','','','','','','','','','','','','','IGST',$invoice_data->igst_amount);
			fputcsv($fp, $row);
			$row=array('','','','','','','','','','','','','','','Grand Total', round($invoice_data->grand_total));
			fputcsv($fp, $row);
			exit;
   ...
   //          header('Content-Disposition: attachment;filename="'.$prefix.$invoiceId.'.xls"');
   ...
   public function payments(){	
		...
		$this->load->helper('form_helper');
		if ($this->session->userdata("userType") == '1') {
			$resAct = $this->db->query("select * from tbl_international_invoice_import where final_invoice = 1");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		} else {
			...
			$resAct = $this->db->query("select * from tbl_international_invoice_import where branch_id='$branch_id' and final_invoice = 1");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		}
		...
		$data['prefix'] = $prefix;
		$this->load->view('admin/booking_master/Import/view_payments', $data);
...
//                $data = array(); 
   ...
   public function save_payment(){
	   ...
	   $resAct = $this->db->query("select * from tbl_international_invoice_import where id = $invoice_id");
	...
	public function payment_history(){		
		...
		$this->load->view('admin/booking_master/Import/view_payment_history', $data);
	}
