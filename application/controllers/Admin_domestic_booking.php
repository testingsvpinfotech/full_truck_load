...
class Admin_domestic_booking extends CI_Controller {
    ...
    public function invoice() {    	
    		...
    		$this->load->helper('form_helper');
    		if ($this->session->userdata("userType") == '1') {
    			$resAct = $this->db->query("select * from tbl_domestic_invoice where final_invoice = 0");
    			...
    			$resAct = $this->db->query("select * from tbl_domestic_invoice where branch_id='$branch_id' and final_invoice = 0 ORDER BY  tbl_domestic_invoice.id desc");
    			if ($resAct->num_rows() > 0) {
    				$data['allpoddata'] = $resAct->result_array();
    			}
    		}
